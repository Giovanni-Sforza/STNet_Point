...
class Trainer:
    ...
    def _setup_optimizer(self):
        ...
        if self.config.train.scheduler.type == "CosineAnnealingLR":
            # 只创建主调度器，它的T_max应该是总epochs减去warmup的epochs
            warmup_epochs = self.config.train.scheduler.get('warmup_epochs', 0) if self.config.train.scheduler.get('use_warmup', False) else 0
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer,
                T_max=self.config.train.max_epochs - warmup_epochs
            )
        else:
    ...
    def _train_one_epoch(self, epoch):
        ...
        # --- 手动Warmup逻辑 ---
        use_warmup = self.config.train.scheduler.get('use_warmup', False)
        warmup_epochs = self.config.train.scheduler.get('warmup_epochs', 0)
        
        if use_warmup and epoch < warmup_epochs:
            # 计算当前warmup epoch的学习率
            target_lr = self.config.train.optimizer.lr
            start_lr = self.config.train.scheduler.get('warmup_start_lr', 1e-7)
            # 线性插值
            current_lr = start_lr + (target_lr - start_lr) * (epoch / warmup_epochs)
        ...
        for i, (data, labels) in enumerate(pbar):
            ...
            # 获取当前实时的学习率来显示
            current_lr_display = self.optimizer.param_groups[0]['lr']
            pbar.set_postfix({'loss': f'{loss.item():.4f}', 'lr': f'{current_lr_display:.6f}'})

        avg_loss = total_loss / len(self.train_loader)
        current_lr_display = self.optimizer.param_groups[0]['lr'] # 获取epoch结束时的lr
        self.writer.add_scalar('Loss/train', avg_loss, epoch)
        self.writer.add_scalar('LearningRate', current_lr_display, epoch)
        self.logger.info(f"Epoch {epoch} Training | Average Loss: {avg_loss:.4f} | End of Epoch LR: {current_lr_display:.6f}")
        
        # --- 修改scheduler的step时机 ---
        # 只有在warmup结束后，才让主scheduler开始工作
