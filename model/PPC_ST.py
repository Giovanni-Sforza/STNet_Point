...
try:
    from extensions.pointnet2.pointnet2_modules import PointnetSAModule
...
class PointPoolingClassifier(nn.Module):
    def __init__(self, num_classes: int, output_dim: int = 512, input_feature_dim: int = 3, num_events: int = 32, feature_out=False):
        ...
        self.settransformer = SetTransformer(dim_input=self.output_dim,num_outputs=1,dim_output=self.output_dim)
    ...
    def forward(self, xyz_cluster, feat_cluster):
        ...
        for i in range(self.num_events):
            ...
            #print(xyz_i.shape,feat_i.shape)
            ...
            #x_i_squeezed = x_i.squeeze(1)
            #event_features_i = torch.cat([x_i_squeezed,h_i_squeezed],dim=1)
        ...
        cls_output = self.settransformer(stacked_features)
...
def create_model(config):
    ...
    model_name = config.get('model_name', 'PPC_ST')
    if model_name == 'PPC_ST':
