...
class FineTuningTrainer:
    def __init__(self, config, pretrained_checkpoint_path=None):
        ...
        self.accumulation_steps = 2 # 在这里定义累积步数
        self.current_iter = 0
...
# --- 你的原始导入和类定义 ---
# (这里我省略了你所有的类定义代码，因为它们不需要修改，直接复制粘贴过来即可)
# from utils.STNet_dataset import PairedEventContrastiveDatasetV3
# from model.STNet_point2 import create_model
#
# class FineTuningTrainer:
...
# >>>>>>>>> 关键：假设上面已经定义了 FineTuningTrainer 类和 get_finetune_config 函数 <<<<<<<<<

# --- 新的测试函数 ---
def run_overfit_test(trainer, num_iterations=300):
    """
    对单个固定的batch进行过拟合测试。
    
    参数:
    trainer: 一个已经初始化的 FineTuningTrainer 实例。
    num_iterations: 在单个batch上训练的迭代次数。
    """
    logging.info("==========================================================")
    logging.info("===            STARTING SINGLE BATCH OVERFIT TEST            ===")
    logging.info("==========================================================")

    # 1. 从训练数据加载器中获取一个固定的batch
    logging.info("Fetching a single batch of data for the test...")
    try:
        # 使用你已经创建好的 DataLoader
        fixed_batch_dict = next(iter(trainer.train_data_loader))
        logging.info("Successfully fetched one batch.")
    except Exception as e:
        logging.error(f"Failed to get a batch from the dataloader: {e}")
        return

    # 2. 将模型设置为训练模式
    trainer.spt_model.train()
    
    # 3. 开始在固定batch上进行训练循环
    logging.info(f"Starting training loop for {num_iterations} iterations on the fixed batch.")
    ...
    for i in range(num_iterations):
        ...
        # 使用你已有的处理函数来准备数据
        # 注意：每次循环都使用同一个 fixed_batch_dict
        xyz1, points1, xyz2, points2, target_labels = trainer._process_spt_batch(fixed_batch_dict)
        
        # 确保数据不为空（以防你的_process_spt_batch过滤掉了所有数据）
        if target_labels.numel() == 0:
            logging.error("The fixed batch resulted in zero valid samples after processing. Cannot run test.")
            return

        # 前向传播
        logits = trainer.spt_model((xyz1, points1), (xyz2, points2))
        
        # 计算损失
        loss = trainer.classification_criterion(logits, target_labels)
        
        # 反向传播
        if torch.isfinite(loss):
            loss.backward()
        else:
            logging.warning(f"Iteration {i+1}: Non-finite loss encountered ({loss.item()}). Skipping backward pass.")
            continue # 跳过此次更新
        ...
        if (i + 1) % 10 == 0:
            ...
            accuracy = correct / target_labels.size(0) * 100.0
            
            logging.info(f"Iter [{i+1:03d}/{num_iterations}] -> Loss: {loss.item():.6f}, Accuracy: {accuracy:.2f}%")

    logging.info("==========================================================")
    logging.info("===            SINGLE BATCH OVERFIT TEST FINISHED            ===")
    logging.info("==========================================================")
    logging.info("Check the logs above. A healthy model should show loss consistently decreasing towards zero.")
...
def get_finetune_config():
    ...
    config_dict = {
        ...
        "finetune_data": { # Data specific to this fine-tuning task
            ...
            "val1_split_file": "contra_data_clean_noipad/fine_test_Ru copy.txt", # Added from your prev request
            "val2_split_file": "contra_data_clean_noipad/fine_test_Zr copy.txt", # Added from your prev request
        ...
        "optimizer": { "name": "AdamW", "lr": 0.001, "weight_decay": 0.0001 }, # LR for fine-tuning
        ...
        "training": {
            ...
            "output_dir": "./STNet_debug1_3abstraction_4class", # CHANGE THIS FOR DIFFERENT RUNS
...
if __name__ == "__main__":
    # --- 你的原始设置代码 (日志，随机种子等) ---
    try: 
        ft_config = get_finetune_config()
    except Exception as e: 
        print(f"FATAL: Error loading ft-config: {e}")
        exit(1)

    # 为了测试，我们可以创建一个临时的输出目录
    test_output_dir = "./STNet_debug1_3abstraction_4class"
    ft_config.training.output_dir = test_output_dir # 覆盖配置中的输出目录
    
    os.makedirs(ft_config.training.output_dir, exist_ok=True)
    log_ft_path = os.path.join(ft_config.training.output_dir, "overfit_test.log")
    
    # 配置日志记录器
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                        handlers=[logging.StreamHandler(), logging.FileHandler(log_ft_path)])
    
    logging.info("Using Configuration for Overfit Test:\n%s", OmegaConf.to_yaml(ft_config))
    ...
    try:
        # --- 实例化 Trainer ---
        # 和你原来一样，但我们在这里不关心预训练或恢复
        logging.info("Initializing FineTuningTrainer for the test...")
        trainer_instance = FineTuningTrainer(ft_config, pretrained_checkpoint_path=None)
        
        # --- 运行我们新的测试函数 ---
        run_overfit_test(trainer_instance, num_iterations=300)

    except Exception as e:
        logging.critical("Unhandled exception during the overfit test:", exc_info=True)